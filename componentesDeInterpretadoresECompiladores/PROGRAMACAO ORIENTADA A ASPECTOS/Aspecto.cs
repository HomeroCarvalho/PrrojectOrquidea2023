...
namespace parser
{
    public class Aspecto
    {
        ...
        private Metodo funcaoCorte { get; set; }
        ...
        public Aspecto(string nameAspect, string nomeTipoDoObjeto, string nomeMetodo, Metodo functionCutter, Escopo escopo, TypeAlgoritmInsertion tipoAlgoritmo, string tipoInsercao)
        {
            if (functionCutter == null)
            {
                UtilTokens.WriteAErrorMensage("funcao de corte nao definida, ou com erros de sintaxe.", escopo.codigo, escopo);
                return;
            }
            this.nomeAspecto = "aspect_" + nameAspect;
            this.nomeTipoDoobjeto = nomeTipoDoObjeto;
            ...
            else
            {
                for (int indexClasse = 0; indexClasse < escopo.tabela.GetClasses().Count; indexClasse++) 
                {
                    ...
                    if (umaClasse.GetMetodos() != null)
                        ...
                        {
                            Metodo umMetodo = umaClasse.GetMetodos()[indexMetodo];
                    ...
                    if ((instrucao.expressoes[indexExpressao].Elementos != null) || (instrucao.expressoes[indexExpressao].Elementos.Count > 0))
                        for (int indexSubExpressao = 0; indexSubExpressao < instrucao.expressoes[indexExpressao].Elementos.Count; indexSubExpressao++)
                        {
                            ...
                            if (instrucao.expressoes[indexExpressao].Elementos[indexSubExpressao].GetType() == typeof(ExpressaoObjeto))
                            {
                                ...
                                Objeto objetoToWatch = exprssObject.objectCaller; // nao é preciso verificar o valor do objeto, apenas seu nome.
                ...
                for (int indexExpressao = 0; indexExpressao < instrucao.expressoes.Count; indexExpressao++)
                    for (int indexSubExpressao = 0; indexSubExpressao < instrucao.expressoes[indexExpressao].Elementos.Count; indexSubExpressao++)
                    {
                        if (instrucao.expressoes[indexExpressao].Elementos[indexSubExpressao].GetType() == typeof(ExpressaoChamadaDeMetodo))
                        {
                            ...
                            if ((exprssMethodCalling.funcao !=null) &&
                                (exprssMethodCalling.objectCaller!=null) &&
                                (exprssMethodCalling.funcao.nome == nomeMetodo) &&
        ...
        public Instrucao BuildCallingMethodCut(ExpressaoObjeto instanciaObjeto)
        {
            ...
            Objeto obj_caller = new Objeto("private", instanciaObjeto.GetTipoExpressao(), "obj_Aspecto_" + rnd.Next(100000), null);
            ExpressaoChamadaDeMetodo expresssaoChamada = new ExpressaoChamadaDeMetodo(obj_caller, funcaoCorte, new List<Expressao>());
            ExpressaoElemento expressaoNomeobjetoMonitorado = new ExpressaoElemento(instanciaObjeto.objectCaller.GetNome());
