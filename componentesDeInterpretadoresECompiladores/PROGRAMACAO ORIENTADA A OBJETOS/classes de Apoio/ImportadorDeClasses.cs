...
namespace parser
{
    ...
    {
        private Assembly assemblyToImporter
        { 
            get;
            set;
        }
        ...
        public ImportadorDeClasses(string aplicacaoEXEouBibliotecaDLL)
        {
            ...
            this.assemblyToImporter = Assembly.LoadFile(Path.GetFullPath(aplicacaoEXEouBibliotecaDLL));
            this.ImportAllClassesFromAssembly();

        
        }
        public void ImportLibrary(string nameLibary, string pathFileLibrary)
        {
        
            
            this.assemblyToImporter = Assembly.LoadFile(pathFileLibrary);
            Type[] classesDaAssembly = this.assemblyToImporter.GetTypes();
            foreach (Type umaClasseDaAssembly in classesDaAssembly)
                if (umaClasseDaAssembly.Name==nameLibary)
                {
        ...
        private void TransfereMetodosEPropriedadesParaOReposicorioDeClassesOrquidea(Type myType)
        {
            ...
            List<Metodo> metodosParaOrquidea = new List<Metodo>();
            List<Metodo> construtoresParaOrquidea = new List<Metodo>();
        ...
        {
            classe.construtores = new List<Metodo>();
            foreach (var umConstrutor in infoConstrutores)
            {
                ...
                classe.construtores.Add(new Metodo(umConstrutor.DeclaringType.Name, acessor, umConstrutor.Name, umConstrutor, umConstrutor.DeclaringType.Name, null, parametros.ToArray()));
            }

        }

        private void RegistraMetodosImportados(MethodInfo[] infoMetodos, List<Metodo> metodos)
        {

            List<Objeto> parametros = new List<Objeto>();
            foreach(MethodInfo umMetodo in infoMetodos)
            {
                // esvazia a lista de parãmetros, para obter os parâmetros do próximo método importado.
                parametros.Clear();
                ...
                metodos.Add(new Metodo(umMetodo.DeclaringType.Name, acessor, umMetodo.Name, umMetodo, umMetodo.DeclaringType.Name, parametros.ToArray()));
